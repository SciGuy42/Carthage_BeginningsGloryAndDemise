...
Trigger.prototype.CorralCheck = function(data)
{
	...
	for (let c of corrals)
	{
		...
		cmpUnitAI.Walk(cmpTargetPosition.x,cmpTargetPosition.z,false,true);
	}
...
Trigger.prototype.IdleUnitCheck = function(data)
{
	for (let p of [7])
	{
		...
		//decide what to do with each idle unit
		if (idle_units.length > 0)
		{
			//make list of celtic cc's
			let ccs = [];
			for (let c_playeer of [2,4,5])
			{
				ccs = ccs.concat(TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(c_playeer),"CivilCentre").filter(TriggerHelper.IsInWorld));
			}
			
			warn("found "+ccs.length+" ccs");
			if (ccs.length < 1)
				return;
			
			for (let u of idle_units)
			{
				//find which cc is closest
				let closestTarget;
				let minDistance = Infinity;
				
				for (let target of ccs)
				{
					if (!TriggerHelper.IsInWorld(target))
						continue;

					let targetDistance = PositionHelper.DistanceBetweenEntities(u, target);
					if (targetDistance < minDistance)
					{
						closestTarget = target;
						minDistance = targetDistance;
				...
				//next, find who owns that target
				var cmpOwnership = Engine.QueryInterface(closestTarget, IID_Ownership);
				let celtic_owner = cmpOwnership.GetOwner();
				
				//make patrol
				let patrol_sites = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(celtic_owner),"Structure").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.GarrisonEntities = function(data)
{
	...
	for (let p of [2,4,5])
	{
		...
		for (let c of towers)
		{
			...
			let archers_e = TriggerHelper.SpawnUnits(c, "units/gaul/champion_fanatic",5,p);
		...
		{
			//spawn the garrison inside the tower
			let archers_e = TriggerHelper.SpawnUnits(c, "units/gaul/champion_fanatic",20,p);
...
{
	//if gaia soldier, then the rest of his group attacks
	...
	if (data.from == 0 && data.to == 1)
	{
		//check if arsenal
		let id = Engine.QueryInterface(data.entity, IID_Identity);
		if (id && id.visibleClassesList.indexOf("Trade") >= 0)
		{
			//spawn ram
			let ram = TriggerHelper.SpawnUnits(data.entity, "units/rome/support_trader",6,1);

		}
		if (id && id.visibleClassesList.indexOf("Temple") >= 0 && data.from == 0)
		{
			let traders = TriggerHelper.SpawnUnits(data.entity, "units/maur/hero_chamakya",1,1);
			
			this.ShowText("Inside the temple, you find a healer from a land far away. He is happy to have been rescued from bandits and will offer his services for the time being.","Great!","Also great!");
			
		}
	}
	
	if (data.from == 6 && (data.to == -1 || data.to == 1))
	{
		let ships = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(6), "Ship").filter(TriggerHelper.IsInWorld);
		let docks = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(6), "Dock").filter(TriggerHelper.IsInWorld);
		
		if (this.seaPeopleAttackTriggered == false)
		{
			if (ships.length == 0 || docks.length == 0)
			{
				this.seaPeopleAttackTriggered = true;
				this.DoAfterDelay(5 * 1000,"SpawnNavalAttack",null);
...
Trigger.prototype.IntervalSpawnGuards = function(data)
{
	...
	let pop_cap = 300; //100 per celtic base
	...
	if (soldiers.length < pop_cap)
	{
		for (let p of [2,4,5])
		{
			...
			for (let i = 0; i < size; i ++)
			{
				let inf_templates = ["units/gaul/champion_fanatic","units/gaul/champion_infantry_swordsman","units/gaul/infantry_spearman_a","units/gaul/infantry_slinger_a","units/gaul/infantry_javelineer_a"];
...
Trigger.prototype.SpawnAnimalHunters = function(data)
{
	...
	{
		let size = pickRandom([1,1,1,2,2,3]);
	...
	this.DoAfterDelay(120 * 1000,"SpawnAnimalHunters",null);
...
Trigger.prototype.SpawnNavalAttack = function(data)
{
	...
	if (docks.length <  1 && ships.length < 1)
		return;
	...
	{
		//find spawn site
		let site = pickRandom(this.GetTriggerPoints(triggerPointsShipAttack));
	...
	else if (targets_ship.length > 0)
	{
		let target = pickRandom(targets_ship);
		for (let attacker of attacker_ships)
		{
			let cmpUnitAI = Engine.QueryInterface(attacker, IID_UnitAI);
			cmpUnitAI.Attack(target);
...
Trigger.prototype.SpawnDorianAttack = function(data)
{
	...
	{
		warn("dorian attack");
		let num_squads = this.dorianNumSquads;
		let squad_size = this.dorianSquadSize;
...
Trigger.prototype.IntervalActionTraders = function(data)
{
	...
	for (let p of [2,4,5])
	{
		...
		if (traders_e.length < 7)
		{
			...
			let markets_others = [];
			for (let p_other of [2,4,5])
			{
				if (p != p_other)
				{
					markets_others = markets_others.concat(TriggerHelper.MatchEntitiesByClass( TriggerHelper.GetEntitiesByPlayer(p_other), "Trade").filter(TriggerHelper.IsInWorld));
	...
	for (let p of [2,6])
	{
		...
		{
			//make list of own markets
			let markets_e = TriggerHelper.MatchEntitiesByClass( TriggerHelper.GetEntitiesByPlayer(p), "Dock").filter(TriggerHelper.IsInWorld);
			
			//make list of possible other markets
			let markets_others = [];
			for (let p_other of [2,6])
			{
				if (p != p_other)
				{
					markets_others = markets_others.concat(TriggerHelper.MatchEntitiesByClass( TriggerHelper.GetEntitiesByPlayer(p_other), "Dock").filter(TriggerHelper.IsInWorld));
			...
			if (markets_e.length  > 0 && markets_others.length > 0)
			{
				...
				//spawn
				let site = pickRandom(markets_e);
				let trader_i = TriggerHelper.SpawnUnits(site,"units/pers/ship_merchant",1,p);
...

{
	...
	cmpTrigger.seaPeopleAttackTriggered = false;
	...
	//init gaia clusters
	cmpTrigger.DoAfterDelay(2 * 1000,"InitGaiaClusters",null);

	//disable templates, tech, etc.
	...
	for (let p of [1,2,3,4,5,6,7])
	{
		...
		if (p == 1)
		{
			...
			cmpPlayer.SetPopulationBonuses(100); //eventually we have to build houses for more troops

		}
		
		//celtic elites are stronger
		if (p == 7)
		{
		...
		//modifiers
		if (p == 6) //sea peoples are excellent traders
		{
			cmpModifiersManager.AddModifiers("Trade benefit", {
					"Trader/GainMultiplier": [{ "affects": ["Trader"], "multiply": 8.0}],
			}, cmpPlayer.entity);
		}
		else { //everyone else also get trading bonus
			cmpModifiersManager.AddModifiers("Trade benefit", {
					"Trader/GainMultiplier": [{ "affects": ["Trader"], "multiply": 2.0}],
			}, cmpPlayer.entity);
			
			cmpModifiersManager.AddModifiers("Trade ship benefit", {
					"Trader/GainMultiplier": [{ "affects": ["Trader+Ship"], "multiply": 2.0}], //ships get even extra
			}, cmpPlayer.entity);
		}
	...
	//sea peoples are neutral to celts
	for (let p of [6])
	{
		let cmpPlayer = QueryPlayerIDInterface(p);
		for (let p_other of [2,4,5,7])
		{
